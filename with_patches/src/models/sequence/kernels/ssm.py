...
class SSMKernel(Kernel):
    ...
    def init_ssm_dplr(self):
        ...
        if self.deterministic:
            ...
            #TODO: Delete, just reducing the number of parameters for debugging

        elif True:
            C = torch.randn(self.channels, self.n_ssm, self.N//2, dtype=self.cdtype)
        else:
...
class SSMKernelDiag(SSMKernel):
    ...
    def register_params(self, A, B, C, inv_dt, P):
        ...
        #TODO: DELETE THIS
        C = C.expand(torch.broadcast_shapes(C.shape, (1, self.n_ssm, self.N))) # (C, H, N)  # TODO originally this was only in DPLR, check safe for Diag
...
class SSMKernelDPLR(SSMKernelDiag):
    ...
    def _setup_C(self, L):
        ...
        C = _r2c(self.C)
        #TODO: DELETE THIS
        # repeat C on axis 1 self.repeat times
        orig_shape = C.shape[1]
        C = C.repeat(1, self.repeat, 1) # (C H N)
        ...
        self.C.copy_(_c2r(C_[..., :orig_shape,:]))
