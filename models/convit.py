...
class GPSA(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
                 locality_strength=1., use_local_init=True, args=None):
        super().__init__()
        ...
        if args.ema == 'ssm_2d':
            self.move = TwoDimensionalSSM(self.dim, ndim=args.ndim, truncation=None, L=args.num_patches, args=args)
        elif args.ema == 's4nd':
    ...
    def forward(self, x):
        ...
        x = rearrange(self.move(rearrange(x, 'b l h -> l b h')), 'l b h -> b l h')
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, use_gpsa=True, args=None, **kwargs):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.use_gpsa = use_gpsa
        if self.use_gpsa:
            self.attn = GPSA(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop,
                             proj_drop=drop, args=args, **kwargs)
        else:
...
class ConvitVisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=48, depth=12,
                 ...
                 local_up_to_layer=10, locality_strength=1., use_pos_embed=True, args=None):
        super().__init__()
