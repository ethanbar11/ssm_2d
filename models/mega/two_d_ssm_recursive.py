...
class TwoDimensionalSSM(nn.Module):
    def __init__(
        ...
        if self.is_complex:
            self.one_matrix_only_B_with_coeffs = self.one_matrix_only_B_with_coeffs.unsqueeze(-1)

            # Create a tensor of zeros with the same shape as x_unsqueezed
            zeros = torch.zeros_like(self.one_matrix_only_B_with_coeffs)

            # Concatenate along the last dimension to get a tensor of shape (a, b, 2)
            self.one_matrix_only_B_with_coeffs = torch.cat((self.one_matrix_only_B_with_coeffs, zeros), -1)
            self.one_matrix_only_B_with_coeffs = _r2c(self.one_matrix_only_B_with_coeffs)
