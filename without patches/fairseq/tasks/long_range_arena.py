...
from fairseq.data.nested_dictionary_dataset import CIFAR10ColoredNestedDictionaryDataset
...
@register_task('cifar10c')
class CIFAR10C(FairseqTask):
    """
    CifAR10C task, which I created here because this repo meant for text and
    I'm trying to make the circle a rectangle.
    ...
    def __init__(self, args):
        ...
        if not hasattr(args, 'max_positions'):
            ...
            self._max_positions = args.max_positions
        args.tokens_per_sample = self._max_positions
        self.normalization = ((0.507, 0.4865, 0.4409), (
        0.2673, 0.2564, 0.2762)) if args.pixel_normalization is None else args.pixel_normalization
        # self.normalization = (0.5, 0.5) if args.pixel_normalization is None else args.pixel_normalization
        assert len(self.normalization) == 2
    ...
    def load_dataset(self, split, combine=False, **kwargs):
        dataset = CIFAR10ColoredNestedDictionaryDataset(split)
        with data_utils.numpy_seed(self.args.seed):
            shuffle = np.random.permutation(int(len(dataset)))
        if self.args.no_shuffle:
            dataset = dataset
        else:
            dataset = SortDataset(
