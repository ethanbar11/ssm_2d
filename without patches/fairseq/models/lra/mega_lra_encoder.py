...
from fairseq.modules.real_number_embedding import CIFARColoredEmbedding


def save_heatmap(x, title, path):
    import matplotlib.pyplot as plt
    import seaborn as sns
    sv = sns.heatmap(x.cpu().detach().numpy(),cbar=False)
    sv.axis('off')
    figure = sv.get_figure()
    figure.savefig(path,bbox_inches='tight', pad_inches=0.01, dpi=400)
    # plt.show()
    figure.clf()
...
class MegaLRAEncoder(nn.Module):
    ...
    def __init__(
        ...
        assert embedding_type in ['sparse', 'linear','cifar_embedding']
        ...
        self.should_save_kernels_and_exit = args.save_kernels_and_exit
        self.layers.extend([
    ...
    def build_embedding(self, embedding_type, embedding_dim, vocab_size, padding_idx):
        if embedding_type == 'sparse':
            ...
            return embed_tokens
        elif embedding_type == 'cifar_embedding':
            embed_tokens =  CIFARColoredEmbedding(embedding_dim)
            return embed_tokens
        else:
    ...
    def forward(
        ...
        if self.chunk_size > 0 and seq_len > self.chunk_size and seq_len % self.chunk_size != 0:
            assert self.embedding_type == 'sparse', 'for image the sequence length {} must be divided by chunk size {}'.format(
        ...
        for i in range(self.num_layers):
            if self.should_save_kernels_and_exit:
                with torch.no_grad():
                    x, _ = self.layers[i](x, x_padding_mask=padding_mask)
            else:
                x, _ = self.layers[i](x, x_padding_mask=padding_mask)
            if self.should_save_kernels_and_exit:
                print('layer {} kernel:'.format(i))
                base_path = '' # The path here should be for saving the kernels
                os.makedirs(base_path, exist_ok=True)
                print(os.path.exists(base_path))
                kernel = self.layers[i].mega_layer.move.last_kernel
                for i in range(kernel.shape[-1]):
                    kernel_i = kernel[:, :, i]
                    title = f'kernel_{i}.jpg'
                    path = os.path.join(base_path, title)
                    print('saving kernel', i, 'to', path)
                    save_heatmap(kernel_i, title, path)
            if not last_state_only:
                inner_states.append(x)
        if self.should_save_kernels_and_exit:
            print('saving kernels done,exiting...')
            exit()
