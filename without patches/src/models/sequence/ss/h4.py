...
class H4(nn.Module):
    def __init__(
        ...
        if layer1 == 'none':
            ...
            self.D1 = nn.Parameter(torch.randn(self.channels, self.H))
        ...
        if divide is not None:
            self.divide_act = Activation(divide)
    ...
    def forward(self, u, state=None, rate=1.0, lengths=None, **kwargs): # absorbs return_output and transformer src mask
        ...
        if self.layer1 == 'none':
            ...
            y = y + contract('bhl,ch->bchl', u1, self.D1)
            assert y.size(1) == 1
            u1 = y.squeeze(1)
        u1 = self.act1(u1)
        u1 = self.glu1(u3) * u1
        ...
        if self.divide is not None:
            y = self.divide_act(u2) / F.softplus(u1)
        else:
            y = u2 * u1
