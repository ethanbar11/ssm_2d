...
class S4ND(nn.Module):
    ...
    def __init__(
        ...
        if verbose:
            import src.utils.train
            log = src.utils.train.get_logger(__name__)
            log.info(f"Constructing S4 (H, N, L) = ({H}, {ndim}, {l_max})")
        ...
        kernel_args_without_class_name = {k: v for k, v in kernel_args.items() if k != 'class_name'}
        self.kernel = nn.ModuleList([
            SSKernel(self.h, N=self.n, L=L, channels=channels, verbose=verbose, **kernel_args_without_class_name)
            for L in self.l_max
        ])
    ...
    def forward(self, u, rate=1.0, state=None, **kwargs):  # absorbs return_output and transformer src mask
        ...
        rate=1.0
        state=None
        image_size = int(math.sqrt(u.shape[0]))
        u = rearrange(u, "(l1 l2) b h -> b l1 l2 h", l1=image_size, l2=image_size)
