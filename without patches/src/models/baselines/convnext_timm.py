...
from src.models.nn.components import TransposedLinear
...
def convnext_timm_tiny_s4nd_2d_to_3d(model, state_dict, ignore_head=True, jank=False):
    ...
    for key in sorted(model_scratch_params_dict.keys()):
        ...
        if '.kernel.0.kernel.' in key:
            new_state_dict[key_with_prefix] = model_scratch_params_dict[key]

        elif '.kernel.1.kernel.' in key:
            # This is the 1st kernel --> 0th kernel from pretrained model
            new_state_dict[key_with_prefix] = state_dict[old_key_with_prefix.replace(".kernel.1.kernel.", ".kernel.0.kernel.")]
        elif '.kernel.2.kernel.' in key:
            # print("FOUND .kernel.2.kernel., putting kernel 1 into kernel 2", key)
            new_state_dict[key_with_prefix] = state_dict[old_key_with_prefix.replace(".kernel.2.kernel.", ".kernel.1.kernel.")]
        elif ignore_head and 'head' in key:
            # ignore head weights
            print("found head key / parameter, ignore", key)
            new_state_dict[key_with_prefix] = model_scratch_params_dict[key]
        # keys match
            ...
            # this covers cases where we did not use s4 in downsample or the stem
            # breakpoint()
...
if __name__ == '__main__':
    model = convnext_tiny(
        ...
        stem_layer=dict(dt_min=0.1, dt_max=1.0, measure='fourier'),
        layer=dict(
            _name_='s4nd',
            bidirectional=True,
            measure='fourier',
